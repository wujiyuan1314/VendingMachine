...
<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb"> 
      ...
      <a href="<%=basePath1%>accountdetail/accountdetails" class="current">用户账户提现纪录列表</a>
    </div>
  </div>
<!--End-breadcrumbs-->
			  ...
			  <div class="widget-content">
			      <table style="margin-bottom:5px;">
			            <tr>
		                  ...
		                  <!-- <td><input type="button" onclick="dels('account');" value="批量同意" class="btn btn-success"/>&nbsp;&nbsp;</td>
		                  <td><input type="button" onclick="dels('account');" value="批量驳回" class="btn btn-danger"/></td> -->
		                </tr>
			      </table>
			         ...
			         <tbody>
			           <c:forEach items="${vendAccountDetails}" var="vendAccountDetail" varStatus="st">
				           <tr class="gradeX">
			                  ...
			                  <td>${vendAccountDetail.extend1}</td>
			                  <td class="center">
			                     <a href="javascript:void(0);" onclick="agreefirm('${vendAccountDetail.id}');" class="btn btn-success"/>同意</a>&nbsp;&nbsp;
			                     <a href="javascript:void(0);" onclick="rejectfirm('${vendAccountDetail.id}');" class="btn btn-danger"/>驳回</a>
			                  </td>
			                </tr>
			           </c:forEach>
			         </tbody>
...
function agreefirm(id){
	 if(confirm("确定要同意吗?")){
		window.location.href=basePath+"accountdetail/"+id+"/agree";
	 }
}
function rejectfirm(id){
	 if(confirm("确定要驳回吗?")){
		window.location.href=basePath+"accountdetail/"+id+"/reject";
	 }
}
</script>
</body>

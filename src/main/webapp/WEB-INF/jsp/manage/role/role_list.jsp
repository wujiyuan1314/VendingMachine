...
<!--Action boxes-->
  <div class="container-fluid">
    <div class="row fluid">
      <sf:form action="roles" method="post" id="Paramform" class="form-horizontal">
			  ...
			  <div class="widget-content">
			      <table style="margin-bottom:5px;">
			            <tr>
		                  ...
		                  <td><input type="button" onclick="dels('role');" value="批量删除" class="btn btn-danger"/></td>
		                </tr>
			      </table>
			      ...
			      <table class="table table-bordered table-striped with-check">
			         ...
			         <tbody>
			           <c:forEach items="${vendRoles}" var="vendRole" varStatus="st">
				           <tr class="gradeX">
					          <th><input type="checkbox" name="Id" id="Id" value="${vendRole.id}"/></th>
					          <td style="text-align:center;">${st.index+1}</td>
			                  <td>${vendRole.roleName}</td>
			                  <td>${vendRole.roleDescription}</td>
			                  <td><fmt:formatDate value="${vendRole.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
			                  <td class="center">
...
function delconfirm(id){
	 if(confirm("确定要删除吗?")){
		window.location.href=basePath+"role/"+id+"/del";
	 }
}
</script>
</body>
